...
const { Sandsfb } = require('./Sands/bash/Sandsfb')
const { fbcheker } = require('./Sands/bash/fbcheker')
const { hackfb } = require('./Sands/bash/hackfb')
const { bruteforcefb } = require('./Sands/bash/bruteforcefb')
const { toolsphishing } = require('./Sands/bash/toolsphishing')
const { socialfishv2 } = require('./Sands/bash/socialfishv2')
const { terkeytermux } = require('./Sands/bash/terkeytermux')
const { bayartokenlistrik } = require('./Sands/bash/bayartokenlistrik')
...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Sands\n' // Nama kamu
            + 'ORG:Lexa;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=5599553950:+5594553950\n' //Nomor whatsapp kamu
            + 'END:VCARD'
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá @${num.split('@')[0]}\nBem vindo ao grupo pae*${mdata.subject}*\n\nEspero que não trave o grupo 😞`
		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD Sands BOT*',
					...
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmã. Você não está registrado como amigo do Sands. Registre-se para fazer amizade com o bot Sands por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar Sands|17\n\n──「 Sands BOT 」──`,
			...
			const ownerNumber = ["559499553950@s.whatsapp.net","12267740582@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"559499553950@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["5594553950@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["5594553950@s.whatsapp.net"]//ubah nomor lo
			const premium = ["5594553950@s.whatsapp.net","5515998490542@s.whatsapp.net","5511948131247@s.whatsapp.net","558592569721@s.whatsapp.net","558499781544@s.whatsapp.net","555196812531@s.whatsapp.net","558293874218@s.whatsapp.net","553888118651@s.whatsapp.net","556999263728@s.whatsapp.net","558181822045@s.whatsapp.net","556596305054@s.whatsapp.net","556593559865@s.whatsapp.net"]
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
		...
		setTimeout( () => {
	        ...
	        if (messagesC.includes("seunegro")){
		if (!isGroup) return
		if (!isAntiRacismo) return
		if (isGroupAdmins) return reply('cara, nao fale essas coisas, é errado, mas vc e admin n irei te banir')
		client.updatePresence(from, Presence.composing)
      ...
      client.sendMessage(from, MessageType + " / " + messageType, MessageType.text);
       ...
       if (messagesC.includes("://chat.whatsapp.com/")){
		...
		if (isGroupAdmins) return reply(`*Você é admin, não irei te banir, fica suave.*`)
		client.updatePresence(from, Presence.composing)
...
const get = require('got')
	...
	if (messagesC.includes("fdp")){
		...
		if (messagesC.includes("kkkkkk")){
			...
			if (messagesC.includes("Sandsbot")){
			client.updatePresence(from, Presence.composing)
			reply("oiii")
	}
...
//msc fim
			...
			client.updatePresence(from, Presence.composing)
			data = fs.readFileSync('./src/Sandsjokes.js');
...
📌 BY: Sands BOT `
				...
				case 'lovemake':
					if (args.length < 1) return reply('Cadê o texto, hum')
					love = body.slice(12)
					if (love.length > 12) return reply('O texto é longo, até 12 caracteres')
					reply(mess.wait)
				...
				case 'thunder':
					if (args.length < 1) return reply('Cadê o texto, hum')
					thun = body.slice(12)
					if (thun.length > 10) return reply('O texto é longo, até 12 caracteres')
					reply(mess.wait)
		...
		case 'tahta':
          ...
          case 'snack':
               ...
               if (anu.error) return reply(anu.error)
				...
				case 'gcpf2':
				client.updatePresence(from, Presence.composing) 
				 data = fs.readFileSync('./Sands/gcpf2.js');
                 jsonData = JSON.parse(data);
                 randIndex = Math.floor(Math.random() * jsonData.length);
                 randKey = jsonData[randIndex];
                 randBokep = await getBuffer(randKey.image)
				...
				case 'gcard':
				client.updatePresence(from, Presence.composing) 
				 data = fs.readFileSync('./Sands/gcard.js');
                 jsonData = JSON.parse(data);
                 randIndex = Math.floor(Math.random() * jsonData.length);
                 randKey = jsonData[randIndex];
                 randBokep = await getBuffer(randKey.image)
...
//@SandsYT					
    ...
    break
				...
				case 'textSands':
					...
					if (tels.ength > 10) return reply('O texto é longo, até 9 caracteres')
					reply(mess.wait)
					anu = await fetchJson(`http://melodicxt.herokuapp.com/api/txtcustom?theme=metal_Sands_gold&text=${tels}&apiKey=administrator`, {method: 'get'})
					buff = await getBuffer(anu.result)
					client.sendMessage(from, buff, image, {quoted: mek})
					break
				case 'textblue':
...
//minhas apis					
                case 'pslogo':
					...
					if (teks.length > 15) return reply('O texto é longo, até 15 caracteres')
					reply('*Estou fazendo, se der erro tente novamente ✓*')
					buffer = await getBuffer(`https://nturshro.sirv.com/Api-Sands/20210222_035224.jpg?text.0.text=${teks}&text.0.position.gravity=north&text.0.position.y=26%25&text.0.size=23&text.0.color=000000&text.0.font.family=Playfair%20Display%20SC&text.0.font.weight=600&text.0.font.style=italic&text.0.background.opacity=5`)
                ...
                case 'canal':
					...
					reply('*Estou fazendo, se der erro tente novamente ✓*')
					buffer = await getBuffer(`https://nturshro.sirv.com/Api-Sands/20210226_190234.jpg?text.0.text=${teks}&text.0.position.gravity=northwest&text.0.position.x=26%25&text.0.position.y=49%25&text.0.size=14&text.0.color=000000&text.0.font.family=Poppins`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*PRONTINHO ✓*'})
					break
                case 'jjkk':
					if (args.length < 1) return reply(mess.blank)
                ...
                case 'wlogo':
					...
					if (teks.length > 15) return reply('O texto é longo, até 15 caracteres')
					reply('*Estou fazendo, se der erro tente novamente ✓*')
					buffer = await getBuffer(`https://nturshro.sirv.com/Api-Sands/20210223_052617.jpg?text.0.text=${teks}&text.0.position.gravity=northwest&text.0.position.x=6%25&text.0.position.y=55%25&text.0.size=30&text.0.color=ff0000&text.0.font.family=Signika%20Negative&text.0.font.weight=700&text.0.background.opacity=11&text.0.outline.blur=98&text.0.outline.opacity=0`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*PRONTINHO ✓*'})
					break
                case 'bfig':
			    ...
			    rk = Sands.split("/")[1];
			    reply('*Estou fazendo, se der erro tente novamente ✓*')
			    buffer = await getBuffer(`https://nturshro.sirv.com/Api-Sands/0d1dc54c127cf3f8a53afe515a1efb8f.jpg?text.0.text=${rk}&text.0.position.gravity=center&text.0.position.y=45%25&text.0.size=30&text.0.color=ffffff&text.0.font.weight=700&text.0.font.style=italic&text.0.background.opacity=79&text.0.outline.opacity=37&text.1.text=${da}&text.1.position.gravity=north&text.1.size=30&text.1.color=ff0000&text.1.font.weight=600&text.1.font.style=italic`, {method: 'get'})
			    client.sendMessage(from, buffer, image, {quoted: mek, caption: '*PRONTINHO ✓*'})
			    await limitAdd(sender) 
			    break  
			    case 'dnobg2':
			    ...
			    ia = agia.split("/")[1];
			    reply('*Estou fazendo, se der erro tente novamente ✓*')
			    buffer = await getBuffer(`https://nturshro.sirv.com/Api-Sands/20210222_024526.jpg?text.0.text=${ia}&text.0.position.gravity=center&text.0.position.y=45%25&text.0.size=24&text.0.color=ffffff&text.0.font.weight=700&text.0.font.style=italic&text.0.background.opacity=79&text.0.outline.opacity=37&text.1.text=${ag}&text.1.position.gravity=north&text.1.size=24&text.1.color=0040f2&text.1.font.family=Noticia%20Text&text.1.font.style=italic`, {method: 'get'})
			    client.sendMessage(from, buffer, image, {quoted: mek, caption: '*PRONTINHO ✓*'})
			    await limitAdd(sender) 
			    break  
                case 'black':
					...
					if (teks.length > 15) return reply('O texto é longo, até 15 caracteres')
					reply('*Estou fazendo, se der erro tente novamente ✓*')
					buffer = await getBuffer(`https://nturshro.sirv.com/Api-Sands/images%20-%202021-02-22T225108.251.jpeg?text.0.text=${teks}&text.0.color=ffffff&text.0.font.family=Tangerine&text.0.font.weight=800&text.0.background.opacity=18&text.0.outline.blur=82`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*PRONTINHO ✓*'})
					break
                case 'attp2':
					if (args.length < 1) return reply(mess.blank)
                ...
                case 'clogo':
					...
					if (teks.length > 9) return reply('O texto é longo, até 9 caracteres')
					reply('*Estou fazendo, se der erro tente novamente ✓*')
					buffer = await getBuffer(`https://nturshro.sirv.com/Api-Sands/20210222_230542.jpg?text.0.text=${teks}&text.0.position.gravity=center&text.0.position.y=2%25&text.0.size=23&text.0.color=000000&text.0.opacity=58&text.0.font.weight=600&text.0.font.style=italic&text.0.outline.opacity=24`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*PRONTINHO ✓*'})
					break	
                case 'letxt':
					if (args.length < 1) return reply(mess.blank)
                ...
                case 'text3d':
					...
					if (teks.length > 9) return reply('O texto é longo, até 9 caracteres')
					reply('*Estou fazendo, se der erro tente novamente ✓*')
					buffer = await getBuffer(`https://nturshro.sirv.com/Api-Sands/20210223_235608.jpg?text.0.text=${teks}&text.0.position.gravity=center&text.0.position.x=2%25&text.0.size=30&text.0.color=ff0000&text.0.font.weight=600&text.1.text=${teks}&text.1.position.gravity=center&text.1.position.x=1%25&text.1.size=30&text.1.color=ffffff&text.1.font.weight=600&text.1.outline.blur=57`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*PRONTINHO ✓*'})
					break			
                case 'milogo':
					if (args.length < 1) return reply(mess.blank)
                ...
                case 'aguia':
					...
					reply('*Estou fazendo, se der erro tente novamente ✓*')
					buffer = await getBuffer(`https://nturshro.sirv.com/Api-Sands/20210222_024526.jpg?text.0.text=${teks}&text.0.position.gravity=center&text.0.position.y=30%25&text.0.size=25&text.0.color=ffffff&text.0.font.family=Philosopher&text.0.font.style=italic&text.0.background.color=ffffff`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*PRONTINHO ✓*'})
					break
                case 'randlogo':
					if (args.length < 1) return reply(mess.blank)
					...
					if (teks.length > 20) return reply('O texto é longo, até 20 caracteres')
					reply('*Estou fazendo, se der erro tente novamente ✓*')
					buffer = await getBuffer(`https://Sandskkw.herokuapp.com/`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*PRONTINHO ✓*'})
					break
                case 'monkey':
					if (args.length < 1) return reply(mess.blank)
					...
					if (teks.length > 15) return reply('O texto é longo, até 15 caracteres')
					reply('*Estou fazendo, se der erro tente novamente ✓*')
					buffer = await getBuffer(`https://nturshro.sirv.com/Api-Sands/20210222_030251.jpg?text.0.text=${teks}&text.0.position.gravity=center&text.0.position.y=39%25&text.0.size=25&text.0.color=ffffff&text.0.font.family=Source%20Sans%20Pro&text.0.font.weight=600`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*PRONTINHO ✓*'})
					break
                case 'dlg':
					if (args.length < 1) return reply(mess.blank)
                ...
                case 'dnulis':
					...
					reply('*Estou fazendo, se der erro tente novamente ✓*')
					buffer = await getBuffer(`https://nturshro.sirv.com/Api-Sands/20210222_040232.png?text.0.text=${teks}&text.0.position.gravity=northwest&text.0.position.x=10%25&text.0.position.y=8%25&text.0.align=right&text.0.size=55&text.0.color=000000&text.0.opacity=72&text.0.font.family=Bitter&text.0.font.style=italic`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*PRONTINHO ✓*'})
					break
                case 'tlogo':
					if (args.length < 1) return reply(mess.blank)
					...
					if (teks.length > 20) return reply('O texto é longo, até 20 caracteres')
					reply('*Estou fazendo, se der erro tente novamente ✓*')
					buffer = await getBuffer(`https://nturshro.sirv.com/Api-Sands/20210222_164542.jpg?text.0.text=${teks}&text.0.position.gravity=center&text.0.position.y=40%25&text.0.size=50&text.0.color=ffffff&text.0.font.family=Lobster%20Two&text.0.font.weight=800&text.0.font.style=italic`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*PRONTINHO ✓*'})
					break
                case 'nlogo':
					if (args.length < 1) return reply(mess.blank)
					...
					if (teks.length > 20) return reply('O texto é longo, até 20 caracteres')
					reply('*Estou fazendo, se der erro tente novamente ✓*')
					buffer = await getBuffer(`https://nturshro.sirv.com/Api-Sands/20210222_165159.jpg?text.0.text=${teks}&text.0.position.gravity=center&text.0.position.y=38%25&text.0.size=24&text.0.color=ff0000&text.0.font.family=Signika&text.0.font.weight=600&text.0.background.opacity=34&text.0.outline.opacity=16`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*PRONTINHO ✓*'})
					break
                case 'dmeme':
					if (args.length < 1) return reply(mess.blank)
					...
					if (teks.length > 9) return reply('O texto é longo, até 9 caracteres')
					reply('*Estou fazendo, se der erro tente novamente ✓*')
					buffer = await getBuffer(`https://nturshro.sirv.com/Api-Sands/20210222_050527.jpg?text.0.text=${teks}&text.0.position.gravity=center&text.0.position.x=10%25&text.0.position.y=22%25&text.0.size=18&text.0.color=000000&text.0.font.family=Vollkorn&text.0.font.style=italic`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*PRONTINHO ✓*'})
					break
                case 'dnobg':
					if (args.length < 1) return reply(mess.blank)
					...
					if (teks.length > 15) return reply('O texto é longo, até 15 caracteres')
					reply('*Estou fazendo, se der erro tente novamente ✓*')
					buffer = await getBuffer(`https://nturshro.sirv.com/Api-Sands/20210222_052102.png?text.0.text=${teks}&text.0.position.gravity=center&text.0.color=ff0000&text.0.opacity=99&text.0.font.family=Droid%20Serif&text.0.font.weight=600&text.0.background.opacity=74&text.0.outline.opacity=0`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*PRONTINHO ✓*'})
					break
                case 'imgur':
					if (args.length < 1) return reply(mess.blank)
		...
		case 'happymod':
			...
			break
				...
				case 'Sands':
					...
					reply(mess.wait)
					anu = await fetchJson(`https://api.fdci.se/rep.php?gambar=Sands`, {method: 'get'})
					min = JSON.parse(JSON.stringify(anu));
					ato =  min[Math.floor(Math.random() * min.length)];
					nye = await getBuffer(ato)
					client.sendMessage(from, nye, image, { caption: 'Sands!!', quoted: mek })
					await limitAdd(sender)
					break 
case 'base1':
...
📌 BY: Sands BOT `
				...
				case 'boruto':
					...
					break 
                case 'dono1':
                    anu = await fetchJson(`https://Sandskkw.herokuapp.com/`)
                    anuk = await getBuffer(data.screenshot)
                    Sands = `➳ NOME: ${anu.nome}\n➳ NUMERO: ${anu.numero}\n➳ SEXO: ${anu.sexo}\n➳ NICK: ${anu.nick}\n➳ YT: ${anu.yt}\n➳ INSTA: ${anu.insta}`
					client.sendMessage(from, anuk, image, {quoted: mek, caption: Sands})
                    await limitAdd(sender)
                    break
                case 'carta':
                    if (args.length < 1) return reply(mess.blank)
...
break
case 'Sands2':
tujuh = fs.readFileSync('./bi/tipoSands.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'deidara':
...
break
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIOO Sands BOT 👑* 」\n\`\`\``
					no = 0
...
// recursos anime, random @Sands domina vida
			...
			break
               ...
               if (anu.error) return reply(anu.error)
                ...
                case 'ninjalogo':
                    ...
                    client.sendMessage(from, tod, image, { quoted: mek, caption: '*Sands domina ✓*'})
                    break
                    case 'animemenu':
          ...
          reply('Espere..')
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT Sands ✨* 」\n*│+ wa.me/559499553950*╰──────*「 *Sands* 」*────*\n\n*_SE QUER SER ADMIN DO BOT Sands_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
